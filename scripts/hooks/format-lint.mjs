...
// 只处理 oxlint/oxfmt 认识的扩展名,跳过 .env / .md / 配置等(否则 oxlint 报 "no target file" 噪音)
const HANDLED = /\.(ts|tsx|js|jsx|mjs|cjs)$/;
if (!HANDLED.test(file)) process.exit(0);
...
// jade 技术栈:oxfmt 原地格式化 + oxlint 检查(都接受单文件路径)
const LINT_CMD = 'npx oxfmt "{{FILE}}" && npx oxlint "{{FILE}}"';
