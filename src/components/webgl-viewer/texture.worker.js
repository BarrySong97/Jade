/**
 * @purpose Web Worker:后台 fetch/解码图片并按 LOD 与瓦片网格切分,产出 ImageBitmap 回传主线程
 * @role    模块的离屏解码线程;由 WebGLImageViewerEngine 经 ?raw 内联为 Blob 启动,响应 load-image/create-tile,回发 image-loaded/tile-created 等消息
 * @deps    Worker 运行时 API(fetch、createImageBitmap、OffscreenCanvas);无模块 import
 * @gotcha  TILE_SIZE 与 WORKER_SIMPLE_LOD_LEVELS 必须与 WebGLImageViewerEngine.ts 保持一致;ImageBitmap 经 transfer 传递,主线程用完须 close();模块说明见 docs/modules/components/README.md
 */
  ...
  switch (type) {
    ...
    case "create-tile": {
      ...
      try {
        ...
        self.postMessage({ type: "tile-created", payload: { key, imageBitmap, lodLevel } }, [
          imageBitmap,
        ]);
      } catch (error) {
