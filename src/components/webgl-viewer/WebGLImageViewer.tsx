/**
 * @purpose WebGL 图像查看器的 React 封装,管理 canvas 生命周期并把 props 装配成引擎配置
 * @role    模块对外组件(由 index.ts 导出);在 useEffect 中实例化/销毁 WebGLImageViewerEngine,通过 ref 暴露 zoomIn/zoomOut/resetView/getScale,并挂载 DebugInfo 面板
 * @deps    react、./WebGLImageViewerEngine、./DebugInfo、./constants(默认配置合并)、./interface(props/ref 类型)
 * @gotcha  config 用 useMemo 计算,变化会重建引擎;debug=true 才创建引擎调试回调与面板;模块说明见 docs/modules/components/README.md
 */
