/**
 * @purpose WebGL 查看器的核心引擎:负责变换矩阵、缩放/平移/双击手势、LOD 与瓦片纹理调度和渲染
 * @role    模块的具体引擎,继承 ImageViewerEngineBase,由 WebGLImageViewer.tsx 实例化;驱动 GL 绘制并把瓦片解码委托给 texture.worker
 * @deps    ./ImageViewerEngineBase(抽象基类)、./shaders(GLSL+编译)、./texture.worker?raw(Web Worker 源码)、./enum、./interface
 * @gotcha  TILE_SIZE/SIMPLE_LOD_LEVELS 必须与 texture.worker.js 保持一致;destroy() 须解绑事件并释放 GL 纹理/buffer 与 worker,否则泄漏;模块说明见 docs/modules/components/README.md
 */
...
export class WebGLImageViewerEngine extends ImageViewerEngineBase {
  ...
  private animate() {
    ...
    this.translateX =
      this.startTranslateX + (this.targetTranslateX - this.startTranslateX) * easedProgress;
    this.translateY =
      this.startTranslateY + (this.targetTranslateY - this.startTranslateY) * easedProgress;
