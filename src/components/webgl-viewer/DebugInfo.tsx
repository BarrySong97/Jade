/**
 * @purpose 开发调试浮层组件,实时展示缩放/变换/LOD/内存/瓦片系统等引擎状态并提供瓦片描边开关
 * @role    模块调试 UI;由 WebGLImageViewer 在 debug 模式下挂载,通过 ref.updateDebugInfo 接收引擎 updateDebugInfo() 推送的数据
 * @deps    react、./interface(DebugInfo 数据类型)
 * @gotcha  纯被动渲染,数据全由引擎主动推送;onToggleOutline 回调上行到引擎 setTileOutlineEnabled;模块说明见 docs/modules/components/README.md
 */
...
const DebugInfoComponent = ({ ref, outlineEnabled, onToggleOutline }: DebugInfoProps) => {
  ...
  const currentOutlineEnabled =
    outlineEnabled !== undefined ? outlineEnabled : (debugInfo.tileOutlinesEnabled ?? false);
