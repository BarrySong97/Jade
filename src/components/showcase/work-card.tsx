...
import { type Work, workCover, workSkipsCoverMorph, displayWorkTitle } from "./works-data";
...
export default function WorkCard({ work, index }: { work: Work; index: number }) {
  ...
  return (
    <article className="works-rise relative" style={{ animationDelay: `${index * 70 + 40}ms` }}>
      <a href={`/products/${work.slug}`} className="absolute inset-0 z-0" aria-label={title} />
