...
- `blogs/` — 文章阅读体验:目录(`Toc.tsx` 桌面 / `MobileToc.tsx` 移动)、代码块(`codeblock.tsx` UI + `code-wrapper-client.tsx` 渐进增强 Shiki `<pre>` + `dynamic-codeblock.tsx` 直传 code)、作者卡 `author-card.tsx`、`blog-image.tsx`(thumbhash 占位 + 懒加载,由图片管线生成,见下方专题);引用卡片 `reference-card.astro`(共用骨架)+ `post-card.astro`(站内,默认紧凑档)/ `link-card.astro`(站外,有 og:image 则大图档),纯 Astro 非岛,只有光标跟随球一小段原生 `<script>`,见下方专题。
